...
In my revious Multidisplay versions this was done by defining *`http://esp.local`* as callback address. This worked fine until Spotify changed its security policies: It is no longer possible to use a local, unencrypted IP address as a direct redirect URI. That change crashed my well-working auth flow, but I found a simple solution. As the address *`http://127.0.0.1`* is still allowed I took this one as callback address being aware that this local address scheme does not work on ESPs. But there was a simple workaround for that: Of course the redirection caused an error message in the browser, but all the relevant data was still in the address bar; you just had to replace the *`127.0.0.1`* with the local ESP IP and keep the rest - and everything worked fine. <br>
